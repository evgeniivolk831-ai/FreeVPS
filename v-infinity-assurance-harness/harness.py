...
def write_artifact(name: str, obj: Dict[str, Any]) -> Path:
    digest = sha256_bytes(canonical(obj))
    wrapped = dict(obj); wrapped["artifact_sha256"] = digest
    path = ARTIFACTS / name
...
def main() -> int:
    ...
    sub = parser.add_subparsers(dest="command", required=True); sub.add_parser("attest")
    h = sub.add_parser("seal-holdout"); h.add_argument("--input", required=True, type=Path)
    e = sub.add_parser("evidence"); e.add_argument("--input", required=True, type=Path); sub.add_parser("self-test")
    args = parser.parse_args()
    path = attest() if args.command == "attest" else seal_holdout(args.input) if args.command == "seal-holdout" else evidence(args.input) if args.command == "evidence" else self_test()
